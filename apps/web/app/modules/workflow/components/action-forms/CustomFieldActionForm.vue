...
const { data, isError, refetch } = useDefaultCustomFieldsQuery();
...
const filtered = computed(() => {
  ...
  return allItems.value.filter((cf) => cf.name.toLowerCase().includes(q));
});
  ...
  () =>
    new Set(
      currentStepActions.value
        .filter((a) => a.targetType === 'custom_field' && a.id !== props.editing?.id)
        .map((a) => a.targetId)
        .filter(Boolean),
    ),
);

function handleSelect(cfId: string, name: string) {
  if (alreadyAddedIds.value.has(cfId)) return;
  ...
  <div class="flex flex-col h-full">
    <div class="p-4 border-b">
      <div class="relative">
        ...
        <Input v-model="search" placeholder="Buscar campo..." class="pl-9" />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <div v-else-if="!allItems.length" class="flex flex-col items-center justify-center h-full py-12 text-center">
        ...
        <NuxtLink to="/settings/custom-fields" class="text-sm text-primary mt-1 hover:underline">
          Ir a configuración
        </NuxtLink>
      </div>
      ...
      <button
        v-for="cf in filtered"
        :key="cf.id"
        class="w-full flex items-start gap-3 px-4 py-3 text-left border-b last:border-0 transition-colors"
        :class="[
          alreadyAddedIds.has(cf.id)
            ? 'opacity-50 cursor-not-allowed bg-muted/30'
            : editing?.targetId === cf.id
              ? 'bg-primary/5 hover:bg-primary/10'
              : 'hover:bg-muted/40',
        ]"
        :disabled="alreadyAddedIds.has(cf.id) || isPending"
        @click="handleSelect(cf.id, cf.name)"
      >
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium truncate">{{ cf.name }}</p>
          <p class="text-xs text-muted-foreground mt-0.5">
            {{ TYPE_LABELS[cf.type] ?? cf.type }}
            <span v-if="cf.automatic" class="ml-1">· IA</span>
          </p>
        </div>
        <Badge v-if="alreadyAddedIds.has(cf.id)" variant="outline" class="text-xs shrink-0 mt-0.5"> Ya añadido </Badge>
        <Badge
          v-else-if="editing?.targetId === cf.id"
          variant="outline"
