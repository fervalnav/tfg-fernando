...
const schema = toTypedSchema(
  z.object({
    ...
    message: requiredString('El mensaje es obligatorio'),
  }),
);
...
onMounted(() => {
  if (props.editing) {
    form.setValues({
      subject: (props.editing.metadata?.['subject'] as string) ?? '',
      message: (props.editing.metadata?.['message'] as string) ?? '',
    });
  }
});

const onSubmit = form.handleSubmit((values) => {
  const metadata = { subject: values.subject, message: values.message };
    ...
    <div class="flex-1 p-4 space-y-4">
      <FormField v-slot="{ componentField }" name="subject">
        <FormItem>
          ...
          <FormControl>
            <Input placeholder="p.ej. Oferta lista para revisión" v-bind="componentField" />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
      ...
      <FormField v-slot="{ componentField }" name="message">
        <FormItem>
          ...
          <FormControl>
            <Textarea
              class="w-full min-h-[100px] rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring resize-y"
