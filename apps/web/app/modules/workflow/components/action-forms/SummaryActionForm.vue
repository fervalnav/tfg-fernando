...
import { useSummaryTemplatesQuery } from '~/modules/summary';
...
const filtered = computed(() => {
  ...
  return allItems.value.filter((t) => t.name.toLowerCase().includes(q) || t.prompt.toLowerCase().includes(q));
});
  ...
  () =>
    new Set(
      currentStepActions.value
        .filter((a) => a.targetType === 'summary' && a.id !== props.editing?.id)
        .map((a) => a.targetId)
        .filter(Boolean),
    ),
);

function handleSelect(templateId: string, name: string) {
  if (alreadyAddedIds.value.has(templateId)) return;
  ...
  <div class="flex flex-col h-full">
    <div class="p-4 border-b">
      <div class="relative">
        ...
        <Input v-model="search" placeholder="Buscar plantilla..." class="pl-9" />
      </div>
    </div>
    ...
    <div class="flex-1 overflow-y-auto">
      ...
      <button
        ...
        :class="[
          alreadyAddedIds.has(template.id)
            ? 'opacity-50 cursor-not-allowed bg-muted/30'
            : editing?.targetId === template.id
              ? 'bg-primary/5 hover:bg-primary/10'
              : 'hover:bg-muted/40',
        ]"
        :disabled="alreadyAddedIds.has(template.id) || isPending"
        @click="handleSelect(template.id, template.name)"
      >
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium truncate">{{ template.name }}</p>
          <p class="text-xs text-muted-foreground truncate mt-0.5">{{ template.prompt }}</p>
        </div>
        <Badge v-if="alreadyAddedIds.has(template.id)" variant="outline" class="text-xs shrink-0 mt-0.5">
          Ya añadida
        </Badge>
        <Badge
          v-else-if="editing?.targetId === template.id"
          variant="outline"
