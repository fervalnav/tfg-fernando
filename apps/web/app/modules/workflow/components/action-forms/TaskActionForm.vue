...
onMounted(() => {
  if (props.editing) {
    form.setValues({ title: (props.editing.metadata?.['title'] as string) ?? '' });
  }
});

const onSubmit = form.handleSubmit((values) => {
  const metadata = { title: values.title };
    ...
    <div class="flex-1 p-4 space-y-4">
      <FormField v-slot="{ componentField }" name="title">
        <FormItem>
          ...
          <FormControl>
            <Input placeholder="p.ej. Revisar documentación técnica" v-bind="componentField" />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
