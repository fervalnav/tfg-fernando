...
de un modelo externo. La selección operativa de proveedor y modelo corresponde
actualmente al desarrollador mediante la configuración de arranque; la
aplicación no ofrece esa elección al usuario.
...
\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{%
    \input{figure-sources/tikz/testing-levels}
  }
...
Las pruebas unitarias y de componentes y su cobertura se regeneraron el 14 de
septiembre de 2026 mediante los ejecutores de Jest y Vitest. La
\Cref{tab:automated-test-results} distingue estos resultados de la campaña
E2E y de integración continua registrada anteriormente. Estas últimas no se
han repetido sobre la versión integrada durante esta revisión: PostgreSQL
local no estaba disponible. Por tanto, su resultado histórico no garantiza
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}p{4.2cm}p{5.8cm}p{3.8cm}@{}}
    ...
    \midrule
    API unitarias y componentes & 41 suites & 158 pruebas superadas \\
    Web unitarias y componentes & 11 archivos & 35 pruebas superadas \\
    API E2E & NestJS y PostgreSQL & 12 de 12; campaña anterior \\
    Navegador E2E & Nuxt, NestJS, PostgreSQL, MinIO y fake de IA & 4 de 4; campaña anterior \\
    Calidad estática & Tipos, lint y construcción & Superada; 13 avisos web no bloqueantes \\
    Integración continua & Tipos, lint, pruebas, cobertura, construcción y API E2E & Superada en campaña anterior \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{@{}lrrrr@{}}
    ...
    \midrule
    API & 63,60\,\% & 62,97\,\% & 40,92\,\% & 38,29\,\% \\
    Web & 9,25\,\% & 8,28\,\% & 4,78\,\% & 9,64\,\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
ejecutor conserva los resultados válidos y reanuda únicamente los no ejecutados,
...
entrada y salida y coste estimado. El coste se calcula por operación con la
metodología de la \Cref{sec:ai-inference-cost}, separando entrada, salida, caché
y tratamiento documental, y después se agrega por expediente y escenario. En
esta campaña no se atribuye ahorro por caché si la respuesta no lo acredita.
El cálculo emplea la tarifa estándar
...
para Flash-Lite, respectivamente \cite{googleGeminiPricing2026}. Las tarifas se
volvieron a verificar en USD en la fuente oficial el 10 de septiembre de 2026.

\subsection{Resultados y límite de cuota}

La cuota disponible de Google permitió completar las 60 salidas de Flash-Lite,
pero se agotó durante la ejecución de Flash y dejó este modelo en 32 de las 60
salidas previstas. El ejecutor respetó el límite de solicitudes por minuto,
esperó ante respuestas de cuota, guardó cada resultado válido y reanudó solo
las combinaciones no ejecutadas. La cuota volvió a agotarse antes de terminar;
por tanto, la diferencia de tamaño no responde a una selección de resultados
ni a fallos descartados del modelo, sino a una restricción externa de acceso al
servicio. Las 92 salidas obtenidas presentan una estructura válida y cuentan
con dos revisiones reconciliadas. La
\Cref{tab:ai-evaluation-results} muestra los resultados observados y explicita
el distinto tamaño de las muestras.

\begin{table}[H]
  \centering
  \scriptsize
  \caption{Resultados de la evaluación de modelos generativos.}
  \label{tab:ai-evaluation-results}
  \resizebox{\textwidth}{!}{%
...
Flash-Lite no se selecciona para resúmenes, a pesar de su media y validez
estructural. Dos respuestas del expediente ficticio de autobuses afirmaron que
...
Flash obtiene una calidad media de 7,75/8 y no presenta alucinaciones críticas
en sus 32 salidas. Al aplicar los criterios por operación se selecciona Flash
para resúmenes y Flash-Lite para preguntas de control, campos personalizados y
decisiones de workflow. Para un único modelo global, los criterios del
experimento recomiendan Flash porque la ausencia de alucinaciones críticas
prevalece sobre la diferencia de coste. Sin embargo, el desarrollador confirma
que el ejemplo de despliegue mantiene Flash-Lite como modelo global. Esta
decisión operativa no modifica la recomendación experimental ni elimina el
riesgo observado en los resúmenes, que requieren revisión humana.
La campaña conserva la limitación de cuota y el tamaño
real de cada muestra, sin sustituir las ejecuciones no realizadas por valores
estimados.
...
seguridad básica de los adjuntos, la recuperación de errores de IA y una
selección de modelo sustentada por operación. No demuestra un rendimiento bajo
...
\begin{longtable}{@{}L{3.6cm}L{3.1cm}L{7.0cm}@{}}
  \caption{Trazabilidad entre riesgos, requisitos y pruebas.}
  \label{tab:testing-traceability} \\
  \toprule
  \textbf{Riesgo} & \textbf{Requisitos} & \textbf{Evidencia principal} \\
  \midrule
  \endfirsthead
  \multicolumn{3}{c}{\tablename\ \thetable{} -- continuación} \\[3pt]
  \toprule
  ...
  \bottomrule
  \endlastfoot
  Sesión inválida y acceso cruzado & RF-001--002; RN-001--002; RN-011 & Servicio de sesión y E2E de autenticación y aislamiento. \\
  Escalada de privilegios & RF-003; RN-003 & Guarda unitaria y matriz E2E de roles y autoeliminación. \\
  Estados incoherentes de proceso & RF-004--005; RF-009; RN-004--005; RN-007 & Entidades, handlers, eventos y recorridos E2E. \\
  Duplicación de cualificación & RF-006; RF-010; RN-006 & Servicios de instanciación y casos de uso de preguntas, campos y resúmenes. \\
  Salida de IA con forma incorrecta & RF-011; RF-015; RN-008; RNF-002; RNF-004 & Esquemas, fake determinista y adaptador de Google. \\
  Documento no válido o inseguro & RF-013--014; RN-002; RN-009; RN-011--012 & Handlers, API E2E y navegador E2E con acceso directo, firmado y caducado. \\
  Solvencia no sustentada & RF-012; RF-016; RI-008 & Banco ficticio, repeticiones, revisiones ciegas y selección por operación. \\
  Error y recuperación de IA & RF-017--018; RN-007 & Persistencia de estado fallido, presentación del error y reintento de la acción. \\
\end{longtable}

Los resultados cierran RF-012 y RF-016 para el alcance definido: el sistema
genera evaluaciones revisables con contexto documental, devuelve proveedor,
modelo, duración y tokens, y el banco compara calidad, latencia y coste para
seleccionar el modelo de cada operación. La persistencia y explotación de
métricas dentro de LIA no forman parte de este cierre y se mantienen como una
evolución de observabilidad, no como una condición de la selección realizada.
